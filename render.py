    ...
    if cfg['effects'].get('caption_solo', True):
        ...
        for _a, _n in zip(_kws, _kws[1:]):
            ...
            if _a['end'] + _AUS > _ns + 1e-3:
                ...
                _n_solo += 1
        # v216 FLIESSTEXT GEGEN FLIESSTEXT. Der ganze Riegel oben haengt an
        # einer KARTE: "solange eine Keyword-Karte steht, raeumt jeder andere
        # Textplan". Zwei Fliesstext-Bloecke gleichzeitig kannte er nicht -
        # und genau das steht in Ismets Render (94 Frames lang 'EVERYONE'S'
        # oben, 'CAPTIONS LOOK...' unten). resolve_overlaps greift dort auch
        # nicht: der prueft nur Bloecke, die NAH BEIEINANDER liegen (< 0.16 H),
        # und diese beiden standen an entgegengesetzten Bildkanten. Damit war
        # 'ein Moment, ein Bild' (v185) fuer den haeufigsten Fall ungedeckt -
        # zum vierten Mal ein Riegel am falschen Gate.
        # Der frueher gestartete raeumt, aber NIE bevor sein letztes Wort
        # gesprochen ist (v187). Reicht das nicht, wartet der spaetere.
        _fl = sorted([p for p in _txt if p.get('kw_i') is None], key=_ct0)
        for _a, _b2 in zip(_fl, _fl[1:]):
            _as2, _bs2 = _ct0(_a), _ct0(_b2)
            _aus_a = float(_a.get('aus', 0.15))
            if _a['end'] + _aus_a <= _bs2 + 1e-3:
                continue                     # sauber nacheinander
            _letzt2 = max((it.get('t', _as2) for it in (_a.get('front') or [])),
                          default=_as2)
            _soll2 = max(_letzt2 + 0.25, _bs2 - _aus_a)
            if _soll2 < _a['end'] - 1e-3:
                _a['end'] = _soll2
                _n_solo += 1
            if _a['end'] + _aus_a > _bs2 + 1e-3:
                # Der Vorgaenger kann nicht weiter raeumen, ohne sein eigenes
                # Schlusswort zu verschlucken - dann faengt der naechste
                # spaeter an und bleibt dafuer entsprechend laenger stehen.
                _spaet2 = _a['end'] + _aus_a
                if _b2['end'] - _spaet2 < _FLOW_MIN:
                    _b2['end'] = _spaet2 + _FLOW_MIN
                _b2['t0'] = _b2['start'] = _spaet2
                _n_solo += 1
        if _n_solo:
